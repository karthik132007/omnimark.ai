...
def make_reevaluate_prompt(question_paper, teacher_model_answer, student_answer, preferences, previous_result):
    prompt = f"""
    You are an expert examiner evaluating student answers fairly and strictly.
    This is a RE-EVALUATION request. Carefully check if you were too harsh or too lenient in the previous evaluation.
    Today's date is {datetime.date.today()}
    ...
    Previous Evaluation Result (For Reference):
    {previous_result}
    ...
    10. Respect max marks from grading preferences.
    11. If unsure due to OCR ambiguity, reduce confidence score.
    12. As this is a re-evaluation, consider the previous result but form your own final judgement.
